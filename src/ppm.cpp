...
void Ppm::initialize_equiprobable() {
	for (int i = 0; i < 256; i++) {
...
void Ppm::update_window(uint8_t atual) {
...
void Ppm::process_symbol(uint8_t atual) {
	...
	if (treino) {
		arvore.update_frequency(contexto_inicial, atual);
	}
	update_context(atual);

	excluidos.clear();
}

/** Decodes one symbol and updates the adaptive PPM model. */
uint8_t Ppm::decode_symbol(ifstream& arquivo_bits) {
	...
	if (treino) {
		arvore.update_frequency(contexto_inicial, (uint8_t)simbolo_decodificado);
	}
	update_context((uint8_t)simbolo_decodificado);
